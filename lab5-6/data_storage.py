...
class DataStorage:
    ...
    def find_sentences_matching_all_words(self, query: str, top_k: int = 5):
        sql = text("""
            SELECT doc_id, sentence_index FROM sentences
            WHERE search_vector @@ plainto_tsquery('russian', :query)
            ORDER BY ts_rank(search_vector, plainto_tsquery('russian', :query)) DESC
            LIMIT :top_k
        """)
        with self.engine.connect() as conn:
            return conn.execute(sql, {"query": query, "top_k": top_k}).fetchall()

    def get_positions_for_word(self, word: str, max_results: int = 30):
        sql = text("""
            SELECT doc_id, sentence_index FROM sentences
            WHERE search_vector @@ plainto_tsquery('russian', :word)
            ORDER BY sentence_index ASC
            LIMIT :limit
        """)
        with self.engine.connect() as conn:
            return conn.execute(sql, {"word": word, "limit": max_results}).fetchall()

    def get_docs_with_all_words(self, words: list, amount_of_words : int = 2):
        if not words: return []
        doc_sets = []
        for w in words:
            if len(w) < 3: continue
            sql = text("SELECT DISTINCT doc_id FROM sentences WHERE search_vector @@ plainto_tsquery('russian', :w)")
            with self.engine.connect() as conn:
                res = conn.execute(sql, {"w": w}).fetchall()
                if not res: return []
                doc_sets.append(set(r[0] for r in res))
        
        if not doc_sets: return []
        common = doc_sets[0]
        for s in doc_sets[1:]: common &= s
        return list(common)[:amount_of_words] 

    def get_sentence_context_window(self, doc_id: int, sentence_index: int, window: int = 10):
        sql = text("""
            SELECT text FROM sentences
            WHERE doc_id = :doc_id
            AND sentence_index BETWEEN :start_idx AND :end_idx
            ORDER BY sentence_index ASC
        """)
        with self.engine.connect() as conn:
