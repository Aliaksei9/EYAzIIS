...
from chat_bot import MusicChatBot
...
class CorpusManager:
    def __init__(self):
        ...
        self.chat_bot = MusicChatBot(self.storage)
    ...
    def handle_menu_selection(self, menu):
        ...
        elif menu == "Семантический анализ":
            self.handle_semantic_analysis()
    ...
    def handle_semantic_analysis(self):
        self.view.render_semantic_analysis()
    ...
    def analyze_document_syntax(self, doc_id):
        ...
        if doc is None:
            return None
        return self.syntax_analyzer.get_sentence_trees(doc, doc_id)
    ...
    def analyze_constituency_tree(self, text):
        return self.syntax_analyzer.analyze_constituency_tree(text)

    def analyze_document_constituency(self, doc_id):
        return self.syntax_analyzer.analyze_document_constituency(doc_id)

    def analyze_document_semantics(self, doc_id):
        """Семантический анализ документа из корпуса."""
        text = self.storage.get_document_text(doc_id)
        if not text:
            return None
        cao_results = self.semantic_analyzer.extract_cao(text)
        causal_chains = self.semantic_analyzer.extract_causal_chains(text)
        return {'cao_triads': cao_results, 'causal_chains': causal_chains, 'text': text}

    def analyze_text_semantics(self, text):
        """Семантический анализ произвольного текста."""
        cao_results = self.semantic_analyzer.extract_cao(text)
        causal_chains = self.semantic_analyzer.extract_causal_chains(text)
        return {'cao_triads': cao_results, 'causal_chains': causal_chains, 'text': text}
    def handle_chat(self):
        self.view.render_chat(self.chat_bot)
